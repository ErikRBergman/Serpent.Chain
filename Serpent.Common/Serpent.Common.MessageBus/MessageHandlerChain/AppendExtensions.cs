...
namespace Serpent.Common.MessageBus
{
    ...
    public static class AppendExtensions
    {
                ...
                innerMessageHandler =>
                    {
                        return async message =>
                            {
                                var chainedMessageTask = innerMessageHandler(message);
                                var newMessage = messageAppendFunc(message);
                                await Task.WhenAll(chainedMessageTask, innerMessageHandler(newMessage));
                            };
                    });
        }
        ...
        private static async Task InnerMessageHandlerAsync<TMessageType>(
            ...
            Func<TMessageType, Task<TMessageType>> messageAppendFunc,
            TMessageType ordinalMessage)
        {
            var newMessage = await messageAppendFunc(ordinalMessage);
