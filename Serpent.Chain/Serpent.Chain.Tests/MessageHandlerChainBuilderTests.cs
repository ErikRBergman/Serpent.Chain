namespace Serpent.Chain.Tests
{
    ...
    public class ChainBuilderTests
    {
        ...
        {
            Assert.Throws<ChainHasAHandlerException>(() => Create.SimpleFunc<int>(b => b.SoftFireAndForget().Handler(m => { }).SoftFireAndForget()));
        }

        [Fact]
        public void NoHandlerException()
        {
            Assert.Throws<ChainHasNoMessageHandlerException>(() => Create.SimpleFunc<int>(b => b.SoftFireAndForget()));
        }
